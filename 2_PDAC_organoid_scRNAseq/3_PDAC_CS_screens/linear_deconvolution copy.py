...
def LM_decon_reg(CompressedScreen, method='ridge', top_alpha=None, top_l1_ratio=None): 
    ...
    if method == 'ols':
        reg = linear_model.LinearRegression(random_state=13, fit_intercept=False)
    ...
    if method == 'ridge':
        if top_alpha != None:
            reg = linear_model.Ridge(alpha=top_alpha, random_state=13, fit_intercept=False)
            reg.fit(CompressedScreen.design_matrix, CompressedScreen.features)
            
        else:
            reg = linear_model.RidgeCV(alphas=np.logspace(-6, 6, 100), random_state=13, fit_intercept=False)
            reg.fit(CompressedScreen.design_matrix, CompressedScreen.features)
    ...
    # Lasso regression
        ...
        if top_alpha != None:
            reg = linear_model.MultiTaskLasso(alpha=top_alpha, random_state=13, fit_intercept=False)
            reg.fit(CompressedScreen.design_matrix, CompressedScreen.features)
            
        else:
            reg = linear_model.MultiTaskLassoCV(n_jobs=-1, random_state=13, fit_intercept=False)
            reg.fit(CompressedScreen.design_matrix, CompressedScreen.features)
            top_alpha = reg.alpha_
    ...
    # ElasticNet regression
        ...
        if top_alpha != None:
            reg = linear_model.MultiTaskElasticNet(l1_ratio=top_l1_ratio, alpha=top_alpha, random_state=13,
                                                   tol=1e-3, max_iter=10000, fit_intercept=False) #increasing max iter and tol
            reg.fit(CompressedScreen.design_matrix, CompressedScreen.features)
            
        else:
            reg = linear_model.MultiTaskElasticNetCV(l1_ratio=[0.01, 0.05, .1, 0.3, .5, .7, .9, .95, .99, 1],
                                                     random_state=13, fit_intercept=False,
                                                     n_jobs=-1, tol=1e-3, max_iter=10000) #increasing max iter and tol
            reg.fit(CompressedScreen.design_matrix, CompressedScreen.features)
            top_alpha = reg.alpha_
